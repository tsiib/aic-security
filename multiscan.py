...
def vul_remed_info(v1, v2, v3):
    print("\t" + bcolors.WARNING + str(tool_resp[v1][0]) + bcolors.ENDC + bcolors.BREAK)
    print(bcolors.BOLD + "Vulnerability Threat Level: " + bcolors.ENDBOLD + vul_info(v2) + bcolors.BREAK)
    print(bcolors.BOLD + "Vulnerability Definition" + bcolors.ENDBOLD + bcolors.BREAK)
    print("\t" + bcolors.BADFAIL + str(tools_fix[v3 - 1][1]) + bcolors.ENDC + bcolors.BREAK)
    print(bcolors.BOLD + "Vulnerability Remediation" + bcolors.ENDBOLD + bcolors.BREAK)
    print("\t" + bcolors.OKGREEN + str(tools_fix[v3 - 1][2]) + bcolors.ENDC + bcolors.BREAK)
...
def logo():
    ...
    print("\n _")
    print("\n _")
    # print("<h2> The Multi-Tool Web Application Scanner </h2>")
    print(bcolors.ENDC)
    ...
    def spinner_task(self):
        try:
            while self.busy:
                ...
                print(bcolors.BG_ERR_TXT + next(self.spinner_generator) + bcolors.ENDC, end=' ')
                sys.stdout.flush()
...
tools_fix = [
    [1,
     "Not a vulnerability, just an informational alert. The host does not have IPv6 support. IPv6 provides more "
     "security as IPSec (responsible for CIA - Confidentiality, Integrity and Availablity) is incorporated into this "
    ...
    [2,
     "Sensitive Information Leakage Detected. The ASP.Net application does not filter out illegal characters in the "
     "URL. The attacker injects a special character (%7C~.aspx) to make the application spit sensitive information "
    ...
    [49,
     "Hackers will be able to read community strings through the service and enumerate quite an information from the "
     "target. Also, there are multiple Remote Code Execution and Denial of Service vulnerabilities related to SNMP "
     "services.",
     "Use a firewall to block the ports from the outside world. The following article gives wide insight on locking "
     "down SNMP service. https://www.techrepublic.com/article/lock-it-down-dont-allow-snmp-to-compromise-network"
     "-security/"],
    [50,
     "Attackers will be able to find the logs and error information generated by the application. They will also be "
     "able to see the status codes that was generated on the application. By combining all these information, "
     "the attacker will be able to leverage an attack.",
...
tools_precheck = [
    ...
    ["golismero"], ["dnsenum"], ["dmitry"], ["davtest"], ["nikto"], ["dnsmap"]
]
...
if len(sys.argv) == 1:
    ...
    if target == '--update' or target == '-u' or target == '--u':
        ...
        if rs_internet_availability == 0:
            print(
                "\t" + bcolors.BG_ERR_TXT + "There seems to be some problem connecting to the internet. Please try "
                                            "again or later." + bcolors.ENDC)
            spinner.stop()
            sys.exit(1)
        ...
        os.system(
            'wget -N https://raw.githubusercontent.com/skavngr/rapidscan/master/rapidscan.py -O rapidscan.py > '
            '/dev/null 2>&1')
        ...
        logo()
       # print(
       #     bcolors.BG_HEAD_TXT + ".[ Checking Available Security Scanning Tools Phase... Initiated. ]" + bcolors.ENDC)
       # unavail_tools = 0
       ...
       # if unavail_tools == 0:
       #     print(
       #         "\t" + bcolors.OKGREEN + ". All Scanning Tools are available. All vulnerability checks will be "
       #                                  "performed by RapidScan." + bcolors.ENDC)
       # else:
       #     print("\t" + bcolors.WARNING + ".Some of these tools " + bcolors.BADFAIL + str(
       #         unavail_tools_names) + bcolors.ENDC + bcolors.WARNING + " are unavailable. RapidScan can still perform "
       #                                                                "tests by excluding these tools from the "
       #                                                                 "tests. Please install these tools to fully "
       #                                                                 "utilize the functionality of RapidScan." +
       #           bcolors.ENDC)
       # print(
       #     bcolors.BG_ENDL_TXT + ".[ Checking Available Security Scanning Tools Phase... Completed. ]" + bcolors.ENDC)
       # print("\n")
       # print(bcolors.BG_HEAD_TXT + ".[ Preliminary Scan Phase Initiated... Loaded " + str(tool_checks) +
       #       " vulnerability checks.  ]" + bcolors.ENDC)

        # while (tool < 1):

        while tool < len(tool_names):
            print(".[" + tool_status[tool][arg3] + tool_status[tool][arg4] + "] Deploying " + str(tool + 1) + "/"
                  + str(tool_checks) + " | " + bcolors.OKBLUE + tool_names[tool][arg2] + bcolors.ENDC, end=' ')
        ...
        if len(rs_vul_list) == 0:
            ...
            with open("RS-Vulnerability-Report", "a") as report:
                ...
                print(
                    ".\t Complete Vulnerability Report for " + bcolors.OKBLUE + target + bcolors.ENDC + " named " + bcolors.OKGREEN + "AIC-Vulnerability-Report" + bcolors.ENDC + " is available under the same directory MultiScan resides.")
        ...
        print(".\t For Debugging Purposes, You can view the complete output generated by all the tools named "
              + bcolors.OKBLUE + "`AIC-Debug-ScanLog`" + bcolors.ENDC)
